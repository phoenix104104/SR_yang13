    ...
    for c = 1:parameter.num_cluster
        ...
        if( exist(feature_filename, 'file') )
            ...
            for i = 1:num_images
                ...
                [LR_features, HR_features, LR_position] = extract_HR_grad_feature(HR_img, parameter);
                
                % PCA
                LR_features = LR_features * M_pca;

                % knn search
                label = knnsearch(cluster_centers, LR_features);

                % extract label
                label_mask = (label == c);
                LR_features = LR_features(label_mask, :);
                HR_features = HR_features(label_mask, :);
