...
method = sprintf('yang13_%s_data%d_LR%d_HR%d_corner%d_center%d_nn%d_lambda%s', ...
...
regressor_filename = fullfile(model_dir, ...
                              sprintf('regressors_%s_data%d_LR%d_HR%d_corner%d_center%d_nn%d_lambda%s.mat', ...
                              parameter.training_dataset, ...
                              parameter.num_patches_for_kmeans, ...
                              parameter.LR_patch_size, ...
                              parameter.HR_patch_size, ...
...
% %% Load old model
% model_filename = sprintf('ClusterCenter_sf%d_sigma%s.mat', scale, num2str(sigma));
% filename = fullfile('Cluster', LR_dir, model_filename);
% fprintf('Load model %s\n', filename);
% loaddata = load(filename, 'clustercenter');
% cluster_centers = loaddata.clustercenter;        %transpose, to make each feature as a column
% 
% model_filename = sprintf('coef_matrix_sf%d_sigma%s.mat', scale, num2str(sigma));
% filename = fullfile('Coef', LR_dir, model_filename);
% fprintf('Load model %s\n', filename);
% loaddata = load(filename, 'coef_matrix');
% coef_matrix = loaddata.coef_matrix;
% 
% regressors = cell(size(coef_matrix, 3), 1);
% for i = 1:size(coef_matrix, 3)
%     regressors{i} = coef_matrix(:, :, i);
% end

% %% create cluster center kdtree
% fprintf('Build kdtree...\n');
% kdtree = vl_kdtreebuild(clustercenter);
...
for i = 1:img_num
    ...
    tic;
    img_yiq_hr(:, :, 1) = SR_simple_function(img_y, parameter, cluster_centers, regressors);
    t_all(i) = toc;
