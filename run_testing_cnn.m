function run_testing_cnn(config_filename)
...
method = sprintf('yang13_%s_data%d_%s_L%d_LR%d_HR%d_corner%d_center%d_nn%d_lambda%s', ...
...
regressor_filename = fullfile(model_dir, ...
                              sprintf('regressors_%s_data%d_%s_L%d_LR%d_HR%d_corner%d_center%d_nn%d_lambda%s.mat', ...
                              parameter.training_dataset, ...
                              parameter.num_patches_for_kmeans, ...
                              parameter.cnn_name, ... 
                              parameter.cnn_target_layer, ...
...
fprintf('Load CNN %s\n', cnn_filename);
parameter.cnn_net = load(cnn_filename);
...
for i = 1:img_num
    ...
    tic;
    img_yiq_hr(:, :, 1) = SR_simple_function_cnn(img_LR, parameter, cluster_centers, regressors);
    t_all(i) = toc;
